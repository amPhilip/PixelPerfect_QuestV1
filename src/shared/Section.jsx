      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8 items-start">
          {data.cards.map((card, index) => {
            // Dynamic margin for reverse order
            const marginClasses = reverse
              ? ["md:mt-0", "md:mt-4", "md:mt-8"]
              : ["md:mt-8", "md:mt-4", "md:mt-0"];
            ...
            return (
              <div
                ...
                className={`group bg-transparent overflow-hidden shadow-lg transform transition-all duration-300 relative 
                  ${
                    index === activeCard && !reverse
                      ? "scale-105"
                      : "hover:scale-105"
                  } 
                  ${marginClasses[index]}`}
                onMouseEnter={() => !reverse && setActiveCard(index)}
              >
                <img
                  src={card.image}
                  alt={card.title}
